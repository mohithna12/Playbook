"""Integration test fixtures -- a real Postgres 16 via testcontainers.

The schema under test is always built by running the actual migrations, never
by ``metadata.create_all()``. Creating tables from ORM metadata would test a
schema that no environment ever runs: partitions, the ``uuidv7()`` function,
expression indexes, and the JSONB defaults all come from migrations.

The container is session-scoped (starting Postgres costs ~2s); each test gets a
